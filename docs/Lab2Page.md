...
### Helpful Links
* [Microphone datasheet](https://www.adafruit.com/product/1063)
...
* The input impedance must be much larger than the IR sensors' output impedance
...
[Here](https://www.youtube.com/watch?v=5Cgi-F-WJ3k) is a video of the light intensity values read out by the Arduino as the distance is varied.
...
void setup() {
  Serial.begin(9600); 
  TIMSK0 = 0;
  ADCSRA = 0xe5; // set the ADC to free running mode and set the prescaler originally to 32
  ADMUX = 0x40;
  DIDR0 = 0x01; 
}
...
void loop() {
  while(1) {
    ...
    for (int i = 0 ; i < 512 ; i += 2) { 
      while(!(ADCSRA & 0x10)); 
      ADCSRA = 0xf5; // reset the ADC and the precaler as well
  }
}
```
...
#### Running the Microphone and 660 Hz Tone Through the ADC
...
if (freq == 18)
  ...
  Serial.println("This is NOT a 660 Hz tone"); 
```
