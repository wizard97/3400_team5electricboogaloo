...
#define TREASURE_IR A5
#define DETECT_THRESH 110
#define LOG_OUT 1 // log output function
#define FFT_N 128 // 256-point fft
...
RF24 radio(9, 10);
const uint64_t pipes[2] = { 0x0000000010LL, 0x0000000011LL }; // radio pipe addresses for the 2 nodes to communicate
  ...
  switch(state) {
    ...
    case(PASSED_LINE):
      ...
      if(popped_frontier == NULL) {
        ...
        if(front_x == popped_frontier->x && front_y == popped_frontier->y) pushStack(&grid[y_coord][x_coord], visited);
...
void transmit(int x, int y, state_dir_t next_dir, bool done) {
  ...
  if(res >= DETECT_THRESH) {
    ...
    if(freq == 17) data.treasure17 = true;
    else if(freq == 12) data.treasure12 = true;
    else data.treasure7 = true;
  ...
  if(role == role_ping_out) {
    ...
    Serial.print("Now sending data.\n");
    bool ok = radio.write( &data, sizeof(data) );

    if(ok) Serial.print("Sent payload %d, suck it trebek.");
    else Serial.print("Failed.\n\r");

    radio.startListening();

    // Wait here until we get a response, or timeout (250 ms)
...
// Helper functions to detect treasures
uint8_t getFreq() {
  int tim_old = TIMSK0;
  int adc_old = ADCSRA;
  int admux_old = ADMUX;
  ...
  if (khz_17 > khz_12 && khz_17 > khz_7) return 17;
  else if (khz_12 > khz_7 && khz_12 > khz_17) return 12;
  else return 7;
}
